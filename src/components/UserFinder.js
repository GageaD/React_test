...
const UserFinder = () => {
   const [filteredUsers, setFilteredUsers] = useState(DATASET);
   ...
   const searchChangeHandler = (event) => {
     // setSearchTerm(event.target.value);
       ...
       const userrs= DATASET.filter((user) => user.toLowerCase().includes(event.target.value.toLowerCase()));
       console.log(userrs)
       setFilteredUsers(
   ...
   return (
     <Fragment>
       <div>
         <input type='search' onChange={searchChangeHandler} className={classes.input} />
       </div>
       <Users users={filteredUsers} pattern={pattern} />
     </Fragment>
   );
 };
